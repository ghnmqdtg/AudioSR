...
def main(args):
    ...
    gt_files = sorted(glob.glob("./gt_wavs/*.wav"))
    # Get the list of upsampled audio files
    gen_files = sorted(glob.glob(f"{args.source_dir}/{args.input_sr}/*.wav"))

    # p374_337.wav is not present in the generated files
    # Remove it from the list of original files
    gt_files = [file for file in gt_files if "p374_337.wav" not in file]

    # Check if the order and filenames are correct
    assert len(gt_files) == len(gen_files)
    for i in range(len(gt_files)):
        assert os.path.basename(gt_files[i]) == os.path.basename(gen_files[i]).replace(
            "_up", ""
        )
    ...
    for i in range(1):
        ...
        for j, (gt_file, gen_file) in enumerate(
            ...
            if i == 0:
                ...
                # Save the generated and original audio files
                torchaudio.save(gen_out, gen, args.target_sr, bits_per_sample=16)
                torchaudio.save(gt_out, gt, args.target_sr, bits_per_sample=16)
